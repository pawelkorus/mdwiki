...
export default class LinkRewriter {
    ...
    static processPageLinks(domElement?: any, baseUrl?: any) {
        ...
        html.find('a').not('#md-menu a').each(function (this:HTMLElement) {
            var $this = $(this);
        ...
        html.find('a, img').each(function (i, e) {
            ...
            if (isImage && !Url.isRelativePath(href))
                return;

            if (!isImage && Url.isGimmickLink(link))
                return;

            function build_link(url:any) {
                if (Url.hasMarkdownFileExtension(url))
                    return '#!' + url;
                else
                    return url;
            }
            ...
            else if (Url.isRelativePath(href))
