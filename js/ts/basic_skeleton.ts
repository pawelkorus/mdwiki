...
export class PageSkeleton {
    ...
    constructor(config: any, domElement: JQuery<HTMLElement>) {
    ...
    private wrapParagraphText() {
        ...
        this.domElement.find('p').each(function(this:HTMLElement) {
            var $p = $(this);
            ...
            var image_children = $p.contents().filter(function (this:HTMLElement) {
                var $child = $(this);
    ...
    private getFloatClass(par:any) {
        var $p = $(par);
        var floatClass = '';

        // reduce content of the paragraph to images
        var nonTextContents = $p.contents().filter(function (this:HTMLElement) {
    ...
    private linkImagesToSelf() {
        var self = this;
        function selectNonLinkedImages(): any {
    ...
    private addInpageAnchors() {
        ...
        // inpage anchor
        function addPilcrow($heading:any, href:string) {
        ...
        function addJumpLinkToTOC($heading:any) {
            if (config.pageMenu && config.pageMenu.disable !== false) return;

            function supportedHeading(heading:any) {
        ...
        this.domElement.find('h1,h2,h3,h4,h5,h6').not('#md-title h1').each(function () {
            ...
            var href = Util.getInpageAnchorHref(text);
